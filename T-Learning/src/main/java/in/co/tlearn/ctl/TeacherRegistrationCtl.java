...
public class TeacherRegistrationCtl extends BaseCtl {
	private static final long serialVersionUID = 1L;
       
	private static Logger log = Logger.getLogger(TeacherRegistrationCtl.class);
...
//		}
		...
		if(DataValidator.isNull(request.getParameter("un"))){
			request.setAttribute("un", PropertyReader.getvalue("error.require", " Teacher Unique Id"));
			pass = false;
		}
		
		/*else if(! DataValidator.isLong(request.getParameter("un"))){
			request.setAttribute("un", " Id should be in numbers (like 101)" );
			pass = false;
		}*/
		
		if(DataValidator.isNull(request.getParameter("qua"))){
			request.setAttribute("qua", PropertyReader.getvalue("error.require", " Qualifications"));
			pass = false;
		}
		
		else if(!DataValidator.isName(request.getParameter("qua"))){
			request.setAttribute("qua", "Invalid Description");
			pass = false;
		}
...
protected BaseBean populateBean(HttpServletRequest request) {
	...
	dto.setRoleid(RoleBean.Teacher);
 ...
 */
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(OP_SIGN_UP.equalsIgnoreCase(op)){
			...
			ServletUtility.setDto(dto, request);
			ServletUtility.setSuccessMessage("Registration successfully", request);
			ServletUtility.forward(ORSView.TEACHER_REGISTRATION_VIEW, request, response);
		}else if (OP_RESET.equalsIgnoreCase(op)) {

			ServletUtility.redirect(ORSView.TEACHER_REGISTRATION_CTL, request, response);
			return;
		}
	...
	protected String getView() {
		...
		return ORSView.TEACHER_REGISTRATION_VIEW;
	}
