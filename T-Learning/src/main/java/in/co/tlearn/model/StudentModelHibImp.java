...
public class StudentModelHibImp implements StudentModelInt {
	...
	public long add(StudentBean dto) throws ApplicationException, DuplicateRecordException {
		...
		CourseBean cdto = model.findByPK(dto.getCourse_id());
		dto.setCourse_name(cdto.getCourse_name());
		  
		StudentBean duplicateName = findByEmailId(dto.getEmailid());
	...
	public StudentBean findByPK(long pk) throws ApplicationException {
		...
		Session session = HibDataSource.getSession();
		StudentBean dto = null;
		try {
			dto = (StudentBean) session.get(StudentBean.class, pk);
